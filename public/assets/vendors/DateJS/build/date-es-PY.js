...
(function () {
			...
			return function (s) {
				...
				for (var i = 0; i < px.length ; i++) {
					...
					if (!last) {
						...
						// if we got a non-empty set as a exam_result ...
						// (otw rx already contains everything we want to match)
						if (p[0].length > 0) {
							// update current exam_result, which is stored in rx ...
							// basically, pick up the remaining text from p[1]
							// and concat the exam_result from p[0] so that we don't
							// get endless nesting ...
	...
	// ignore, which parses a given rule and throws away the exam_result.
